...
\noindent \textasciicircum: a "hat" placed over a letter means that the value is an estimate, prediction, or fitted value derived from sample data.
...
\noindent In probability theory, the law of large numbers is a mathematical law that states that the average of the results obtained from a large number of independent random samples converges to the true value, if it exists. 
...
\noindent Monte Carlo methods, also called the Monte Carlo experiments or Monte Carlo simulations, are a broad class of computational algorithms based on repeated random sampling for obtaining numerical results. 
...
\noindent \textbf{Cauchy Sequence}

\noindent Given a metric space \((M, d)\), a sequence of elements of \(X\), \(x_1, x_2, x_3, \dots\) is Cauchy, 
...
\noindent \textbf{Complete Metric Space}

\noindent In mathematical analysis, a metric space \(M\) is called complete (or a Cauchy space) if every Cauchy sequence of points in \(M\) has a limit that is also in \(M\).
...
\noindent Let \((X, d)\) be a non-empty complete metric space with a contraction mapping \(T:X \rightarrow X\). Then \(T\) admits a unique fixed-point \(x^{*}\) in X, \(T(x^{*})=x^{*}\). 
