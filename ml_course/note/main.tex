...
\noindent \textbf{Distributional shift:}

\noindent Distributional shift happens when training data and test data has a different distribution. This mismatch can lead to a decline in the model's accuracy and reliability.
...
\noindent \(A_t\): action at time step \(t\).

\noindent \(S_t\): (fully observed) state of environment at time step \(t\).
...
\noindent \(v_{\pi}(s)\): value function estimates the expected cumulative future reward. Used to evaluate the goodness or badness of states, 
and select actions based on that. 
For example, for discrete models it can be \(v_{\pi}(s) = E_{\pi}[R_{t + 1} + \gamma R_{t + 2} + \gamma^2 R_{t + 3} + \dots \mid S_t = s]\).

\noindent \(\mathcal{P}, \mathcal{R}\): a model predicts what the environment will do next. 
\(\mathcal{P}\) predicts next state: \(\mathcal{P}_{s s'}^a = P[S_{t + 1} = s' \mid S_t = s, A_t = a]\). 
\(\mathcal{R}\) predicts next reward: \(\mathcal{R}_{s}^a = E[R_{t + 1} \mid S_t = s, A_t = a]\).

\noindent \(\gamma\): discount factor.

\noindent \(r(a_t, s_t)\): reward at state \(s_t\) with action \(a_t\).

\noindent \(c(a_t, s_t)\): cost at state \(s_t\) with action \(a_t\). often the opposite of \(r\).

\noindent \(\pi^*_{\theta}\): optimal policy that gives \(a_t\) for given \(s_t\). Sometimes we assume expert data to be the optimal policy.

\noindent \(\pi_{\theta}\): policy that gives probability of \(a_t\) for given \(s_t\), to maximize cumulative reward \(\sum_{t = 1}^{T}r_t\).

\noindent \(p_{\theta} (s_t, a_t)\): The probability distribution of \(s_t\), \(a_t\) for policy \(\pi_{\theta}\).

\noindent \((s_t, a_t) \sim p_{\theta} (s_t, a_t)\): state and action \((s_t, a_t)\) sampled from distribution \(p_{\theta} (s_t, a_t)\).

\noindent \(p(s_{t+1} \mid s_t, a_t)\): transition probability from state \(s_t\) to \(s_{t + 1}\) given action \(a_t\).

\noindent \(\tau\): a trajectory of states and actions \((s_1, a_1), (s_2, a_2), \dots, (s_T, a_T)\).

\noindent Trajectory probability distribution for \(\tau\):

\[p_{\theta}(\tau) = p(s_1) \prod_{t = 1}^T \pi_{\theta} (a_t \mid s_t) p(s_{t + 1} \mid s_t, a_t) \]

\noindent Objective for reinforcement learning, finite horizon case:

\begin{equation*}
\begin{split}
\theta^*
& = arg \underset{\theta}{max} E_{\tau \sim p_{\theta}(\tau)}[\sum_{t} r(s_t, a_t)] \\
& = arg \underset{\theta}{max} \sum_{t = 1}^{T} E_{(s_t, a_t) \sim p_{\theta}(s_t, a_t)}[r(s_t, a_t)]
\end{split}
\end{equation*}

\noindent Infinite horizon case:

\[\theta^* = arg \underset{\theta}{max} E_{(s, a) \sim p_{\theta}(s, a)}[r(s, a)]\]
...
\noindent The state transition probability is defined by:

\[\mathcal{P}_{ss'} = P[S_{t + 1} = s' \mid S_t = s]\]

\noindent The state transition matrix, where each row of the matrix sums to 1:
...
\noindent A Markov decision process (MDP) is a Markov reward process with decisions. It is an environment in which all states are Markov. 
A Markov decision process is a tuple \(\langle \mathcal{S}, \mathcal{A}, \mathcal{P}, \mathcal{R}, \gamma \rangle\), where:

\bigskip

\noindent \(\mathcal{S}\): a set of states called the state space

\noindent \(\mathcal{A}\): a set of actions called the action space

\noindent \(\mathcal{P}\): a state transition probability matrix, 
\(\mathcal{P}_{ss'}^a = P[S_{t + 1} = s' \mid S_t = s, A_t = a]\)

\noindent \(\mathcal{R}\): the expected immediate reward received after transitioning from state \(S_t\) due to action \(A_t\), 
\(R_s^a = E[R_{t + 1} \mid S_t = s, A_t = a]\)

\noindent \(\gamma\): a discount factor, \(\gamma \in [0, 1]\). 
The discount factor handles the uncertainty of the future reward. Avoids infinite returns in cyclic Markov processes, and brings convenience to mathematical calculations. 
Sometimes immediate rewards may earn more interest than delayed rewards. 

\bigskip

\noindent \(G_t\): the return \(G_t\) is the total discounted reward from time-step \(t\)
...
\noindent \(v(s)\): the value function \(v(s)\) gives the expected return starting from state \(s\).
...
\noindent Here is another derivation (from David Silver's course) of the above formula for \(v(s)\). 
Assume \(s\) is the state on time step \(t\), and \(s'\) is the state on time step \(t + 1\). 
Under condition \(S_t = s\), \(G_{t + 1}\) can be seen as an expectation under condition \(S_{t + 1} = s'\). 
We know that the distribution of \(S_{t + 1}\) is decided by \(S_t\), 
which means \( E[G_{t + 1} \mid S_t = s] = E[E[G_{t + 1} \mid S_{t + 1} = s'] \mid S_t = s]\). 
In this case we get:
...
& = E[R_{t + 1} \mid S_t = s] + \gamma E[E[G_{t + 1} \mid S_{t + 1} = s'] \mid S_t = s] \\
& = E[R_{t + 1} \mid S_t = s] + \gamma E[v(s') \mid S_t = s] \\
& = E[R_{t + 1} + \gamma v(s') \mid S_t = s]
\end{split}
\end{equation*}
