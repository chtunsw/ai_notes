...
\begin{itemize}
    \item It is a mathematical formalism for learning-based decision making. Approach for learning decision making and control from experience.
    \item Reinforcement learning does not assume the data is independently identity distributed. Previous outputs influence future inputs.
    \item Ground truth answer is not known, only know if we succeeded or failed. More generally, we know the reward.
\end{itemize}

\noindent RL optimization:

\begin{itemize}
    \item search: find new patterns that haven't been found before.
    \item learning: use data to extract patterns.
\end{itemize}
...
\noindent \(g\): the goal state for the model to reach.

\noindent \(o_t\): observation at time step \(t\). (not fully observed)

\noindent \(s_t\): state at time step \(t\). (fully observed)
