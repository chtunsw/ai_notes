...
\textbf{Training(Learning) Process:}

\textit{observed data(training set)} $\rightarrow$ \textit{learning algorithm} $\rightarrow$ \textit{h(hypothesis)}

\textbf{Predicting Process:}

\textit{independent variable} $\rightarrow$ \textit{h(hypothesis)} $\rightarrow$ \textit{dependent variable}
...
Draw inferences from data sets consisting of input 
...
\(y\) is a vector of observed values \(y_{i}\ (i=1,\ldots ,n)\) of the variable called the regressand, endogenous variable, response variable, measured variable, criterion variable, or dependent variable.
...
\(\beta\) is a \((p + 1)\)-dimensional parameter vector, where \(\beta_{0}\) is the intercept term (if one is included in the model—otherwise \(\beta\) is p-dimensional). Its elements are known as effects or regression coefficients (although the latter term is sometimes reserved for the estimated effects).

\(\epsilon\) is a vector of values \(\epsilon_{i}\). This part of the model is called the error term, disturbance term, or sometimes noise (in contrast with the ``signal" provided by the rest of the model).

\textbf{Matrix Concepts}

\textbf{identity matrix} \(I\)(or \(I_{n \times n}\)):
...
\textbf{inverse matrix} \(A^{-1}\): If \(A\) is an \(m \times m\) matrix, and if it has an inverse.
\[A \cdot A^{-1} = A^{-1} \cdot A = I\]

\textbf{transpose matrix} \(A^{T}\): Let \(A\) be an \(m \times n\) matrix. Then \(A^T\) is an \(n \times m\) matrix.
\[A^T_{ij} = A_{ji}\]
...
\textbf{mean normalization}: (don't apply to \(x_0\))

\[x_i = \frac{x_i - mean(x_i)}{max(x_i) - min(x_i)}\]

\textbf{standardization}: (don't apply to \(x_0\))
...
Calculate best \(\theta\): \textcolor{orange}{ * maths derivation needed}
...
\textbf{Gradient Descent} vs \textbf{Normal Equation}:
...
\textbf{Vectorized Cost Function}:

\[J(\theta) = \frac{1}{2m} (X\theta - y)^T(X\theta - y)\]

\subsection{Logistic Regression}

\textbf{sigmoid function:} (Logistic Function)
...
\textbf{advanced optimization methods}:
...
\textbf{regularized gradient descent}:
...
\textbf{regularized normal equation}: \textcolor{orange}{ * maths derivation needed}
...
Recall that if \(m < n\), then \(X^TX\) is non-invertible. However, when we add the term \(\lambda L\) (\(\lambda > 0\)), the term \(X^TX + \lambda L\) becomes invertible.
...
\textbf{matrix calculus:} see \href{https://en.wikipedia.org/wiki/Matrix_calculus}{Wikipedia}

\textbf{chaine rule:}

Suppose the variable \(J\) depends on the variables \(\theta_1, \dots, \theta_p\) via the intermediate variable \(z_1, \dots, z_k\).
...
\textbf{chain rule derivation for matrix:}
...
We can see \(\frac{\partial}{\partial \Theta} J(\Theta)\) is hard to get directly. There is an easier way to calculate it. For each training example \((x^{(q)}, y^{(q)}), q \in \{1, \dots, m\}\), define \textbf{loss function}:
...
For convenience, define \textbf{error term}: 
...
\textbf{backpropagation algorithm:} (compute \(\frac{\partial}{\partial \Theta} J(\Theta)\))

training set: \((x^{(q)}, y^{(q)}), q \in \{1, \dots, m\}\)

set \(\Delta_{ij}^{(l)} = 0, l \in \{1, \dots, L - 1\}\)

for \(q \in \{1, \dots, m\}\):

\hspace{.5cm} forward propagation: compute \(a^{(l)}\) for \(l \in \{2, \dots, L\}\)

\hspace{.5cm} compute \(\delta^{(L)} = a^{(L)} - y^{(q)}\)

\hspace{.5cm} for \(l \in \{L - 1, \dots, 2\}\):

\hspace{1cm} compute \(\delta^{(l)} = (\Theta^{(l)})^T \delta^{(l + 1)} \odot (a^{(l)} \odot (1 - a^{(l)}))\)

\hspace{.5cm} for \(l \in \{1, \dots, L - 1\}\):

\hspace{1cm} compute \(\Delta^{(l)} := \Delta^{(l)} + \frac{\partial J}{\partial \Theta^{(l)}} = \Delta^{(l)} + \delta^{(l + 1)}(a^{(l)})^T\)

compute \(\frac{\partial}{\partial \Theta_{ij}^{(l)}} J(\Theta) = D_{ij}^{(l)} = 
\begin{cases}
...
\textbf{random initialization:} (symmetry breaking)
...
Distributional shift happens when training data and test data has a different distribution. This mismatch can lead to a decline in the model's accuracy and reliability.

\begin{center}
    \includegraphics[scale=0.4]{./images/distributional_shift.png}
\end{center}
...
Variance: measures how much a model's predictions varies with different inputs (training datasets).
...
\(\lambda = 0\) causes overfitting. \(\lambda\) too large causes underfitting. To find a good \(\lambda\):
...
\textbf{Precision \& Recall: (deal with skewed classes)}
...
\textbf{root node}: The node on the top.

\textbf{decision node}: The node used to split data.

\textbf{leaf node}: The node on the bottom.

\textbf{node entropy}: Measure of the purity of the split data on nodes. Large if the data is not pure. Small if data is pure. Assume decision tree has \(K\) output classes. For \(i \in \{1, \dots, K\}\), \(p_k\) is the proportion of class \(k\) of all the data examples this node has.

\[{H}(node) = -\sum _{i=1}^{K}p_{k}\log _{2}p_{k}\]

\textbf{information gain}: Reduce of entropy from parent to children nodes. Assume parent node has \(C\) children, \(m_{p}\) data examples, entropy \({H}(p)\). Each child for \(c \in \{1, \dots, C\}\) has \(m_{c}\) data examples, entropy \({H}(c)\).
...
\textbf{node entropy.}: For regression problem, we can use variance of the outputs of the examples on node \(\sigma^{2}(node)\) as the entropy.

\[{H}(node) = \sigma^{2}(node)\]

\textbf{information gain}: Same expression as classification problem, just with different entropy.
...
\textbf{What if feature has multiple possible values?}

Use one-hot encoding to build training set. For example, if \textbf{Ear shape} has 3 possible values: \textbf{Pointy}, \textbf{Floppy}, \textbf{Oval}. We can add three features for each of the possible values to the training set, instead of adding \textbf{Ear shape} as a feature.

\textbf{What if feature is continuous?}

Use ranges of the feature to build training set. For example, \textbf{Weight} has a continuous value. We can add ranges: \(Weight \leq 0.8\), \(0.8 < Weight < 0.9\), \(Weight \geq 0.9\) as features to the training set.
...
Given training set of size \(m\), for \(b \in \{1, \dots, B\}\) (\(B\) means "bags", the number of training sets we want to generate): use sampling with replacement to create a new training set of size \(m\),
...
\(\mu_{k}\): cluster centroid of cluster \(k \in \{1, \dots, K\}\).
...
\textbf{Random initialization}: (reduce the chance of getting local optimum)
...
\textbf{training set}: \(m\) training examples with \(y = 0\) (non-anomalous).

\textbf{cross validation set}: \(m_{cv}\) examples with mostly \(y = 0\) (non-anomalous) and some \(y = 1\) (anomalous).

\textbf{test set}: \(m_{test}\) examples with mostly \(y = 0\) (non-anomalous) and some \(y = 1\) (anomalous).

\textbf{evaluation}: use \(F_1\) score for skewed dataset.

\textbf{Training Process}:

Fit model \(p(x)\) on training set. (get \(\mu, \sigma^2\))

On cross validation/test set, do evaluation and tune \(\epsilon, x_{j}\) to minimize \(F_1\) score. (choose the right threshold and features)

\textbf{Non-gaussian Features}:

\begin{center}
\includegraphics[scale=0.3]{./images/anomaly_detection_features.png}
\end{center}

We want the features fit gaussian distribution well. But features are not always normally distributed. We can process the features to make them normally distributed, for example: use \(log(x_{j} + c)\), \(x_{j}^{\frac{1}{c}}\) instead of \(x_{j}\).
...
\textbf{Anomaly Detection vs. Supervised Learning}:
...
\textbf{predict like/unlike}: (user j on movie i)
...
\(v_{u}\): learned user vector.

\(v_{m}\): learned movie vector.
...
\textbf{How to efficiently find recommendation from
