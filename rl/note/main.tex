...
Reinforcement learning uses the formal framework of Markov decision processes to define the interaction between a learning agent and its environment in terms of states, 
...
The concepts of value and value function are key to most of the reinforcement learning methods. 
...
The kind of model assumed in dynamic programming, estimates of the MDP's dynamics, \(p(s', r \mid s, a)\), is a distribution
model.

There are two fundamental approaches in sequential decision making:
...
\(\mathcal{S}\): set of all nonterminal states. \(\mathcal{S^+}\) represents set of all states including the terminal state.

\(\mathcal{A}\): set of all actions. \(\mathcal{A}(s)\) represents set of all actions available in state \(s\).

\(\mathcal{R}\): set of all possible rewards.

\(t\): discrete time step. \(t = 0, 1, 2, 3, \dots\).

\(O_t\): (not fully observed) observation of environment at time step \(t\).

\(A_t\): action at time step \(t\). \(A_t \in \mathcal{A}\).

\(S_t\): (fully observed) state of environment at time step \(t\). \(S_t \in \mathcal{S}\).

\(S_t^e\): environment state, the environment's private representation. 
...
\(S_t^a\): agent state, the agent's internal representation of the environment. 
...
\textbf{Information state (Markov state)}: \(S_t\) that contains all useful information from the history. 
...
\(R_t\): reward at time step \(t\), typically due, stochastically, to \(S_{t - 1}\) and \(A_{t - 1}\). \(R_t \in \mathcal{R}\).

\(H_t\): history, a trajectory that contains a sequence of observations and actions up to time step \(t\). \(H_t = A_0, O_1, \dots, A_{t - 1}, O_{t}\).

\(\gamma\): discount factor. Determines the importance of future rewards, also to avoid infinite returns.

\(G_t\): return following time \(t\), the cumulative future reward. \(G_t = R_{t + 1} + \gamma R_{t + 2} + \gamma^2 R_{t + 3} + \dots\)

\(p(s', r \mid s, a)\): probability of transition to state \(s'\) with reward \(r\), from state \(s\) and action \(a\). A model of the environment, that completely characterizes the environment's dynamics.
...
\(\pi\): a policy decides agent's behaviour. It maps state to action. 
Deterministic policy: \(a = \pi(s)\). 
Stochastic policy: \(\pi(a \mid s) = P[A_t = a \mid S_t = s]\).

\(\pi_*\): optimal policy that gives best \(a\) for given \(s\).

\(v_{\pi}(s)\): value of state \(s\) under policy \(\pi\), estimates the expected return following state \(s\). 
...
\(q_{*}(s, a)\): value of taking action \(a\) in state \(s\) under the optimal policy \(\pi_*\). Finding \(q_{*}(s, a)\) is the goal of RL tasks.

\(V\), \(V_t\): array estimates of state-value function \(v_{\pi}\) or \(v_{*}\).

\(Q\), \(Q_t\): array estimates of action-value function \(q_{\pi}\) or \(q_{*}\).

\(\bar{V}_{t}(s)\): expected approximate action value, e.g. \(\bar{V}_{t}(s) = \sum_a \pi(a \mid s) Q_t(s, a)\)
...
For episodic tasks, there is a finite time step \(T\) when a terminal state is reached. 
...
\(G_t\): the return \(G_t\) is the total discounted reward from time-step \(t\). Note here \(T = \infty\) or \(\gamma = 1\) is possible (but not both, otherwise return would be infinite).
...
\(v_{\pi}(s)\): state-value function for policy \(\pi\), the expected return when starting in \(s\) and following \(\pi\) thereafter
...
\(q_{\pi}(s, a)\): action-value function for policy \(\pi\), the expected return starting from \(s\), taking the action \(a\), and following \(\pi\) thereafter
...
From the above equations, we can get following relationship between \(v_{\pi}(s)\) and \(q_{\pi}(s, a)\):
...
\(\pi_*\): the optimal policy (at least one, may be more than one). \(\pi \geq \pi'\) if and only if \(v_{\pi}(s) \geq v_{\pi'}(s)\) for all \(s \in \mathcal{S}\). 

\(v_*(s)\): the optimal state-value function, for all \(s \in \mathcal{S}\)

\[v_*(s) = \max_{\pi} v_{\pi}(s) \]

\(q_*(s, a)\): the optimal action-value function, for all \(s \in \mathcal{S}\) and \(a \in \mathcal{A}(s)\)
...
All optimal policies achieve the optimal state-value function and the optimal action-value function, 
for all \(s \in \mathcal{S}\) and \(a \in \mathcal{A}(s)\)

\[v_{\pi_*}(s) = v_*(s) \]
\[q_{\pi_*}(s, a) = q_*(s, a) \]

The Bellman optimality equation for \(v_*\) intuitively expresses the fact that the value of a state under an optimal policy must equal the expected return for the best action from that state:
...
The Bellman optimality equation for \(q_*\), select the best action to maximize the future return:
...
For finite MDPs, the Bellman optimality equation is solvable. 
...
When \(\pi\) is given, if the environment's dynamics \(p(s', r \mid s, a)\) are completely known, 
...
This algorithm is called iterative policy evaluation. The existence and uniqueness of \(v_{\pi}\) are guaranteed as long as either \(\gamma < 1\) or eventual termination is guaranteed from all states under the policy \(\pi\). 
...
To produce each successive approximation, \(v_{k + 1}\) from \(v_{k}\), iterative policy evaluation applies the same operation to each state \(s\): 
...
To write a sequential computer program to implement iterative policy evaluation you would have to use two arrays, one for the old values, \(v_{k}(s)\), 
...
\textbf{Iterative Policy Evaluation (in-place) process}:
...
\hspace{.5cm} \(\Delta \leftarrow 0\)

\hspace{.5cm} Loop for each \(s \in \mathcal{S}\):

\hspace{1cm} \(v \leftarrow V(s)\)

\hspace{1cm} \(V(s) \leftarrow \sum_a \pi(a \mid s) \sum_{s', r} p(s', r \mid s, a) [r + \gamma V(s')]\)

\hspace{1cm} \(\Delta \leftarrow \max(\Delta, \vert v -  V(s) \vert)\)
...
Here is the proof of the policy improvement theorem:
...
Inside the equation, for example, the \(E_{\pi'}[R_{t + 1} + \gamma R_{t + 2} + \gamma^2 v_{\pi}(S_{t + 2}) \mid S_t = s]\) means the expected discounted action-value when starting in state \(s\), 
...
where \(\underset{a}{argmax}\) denotes the value of \(a\) at which the expression that follows is maximized (with ties broken arbitrarily). 
...
In the general case, a stochastic policy \(\pi\) specifies probabilities, \(\pi(a \mid s)\), for taking each action, \(a\), in each state, \(s\). 
...
Each policy is guaranteed to be a strict improvement over the previous one (unless it is already optimal). Because a finite MDP has only a finite number of policies, 
...
\textbf{Policy Iteration (using iterative policy evaluation) for estimating deterministic policy \(\pi \approx \pi_*\)}:

(can extend to stochastic policy with adjustments)

1. Initialization

\hspace{.5cm} \(V(s) \in \mathbb{R}\) and \(\pi(s) \in \mathcal{A}(s)\) arbitrarily for all \(s \in \mathcal{S}\)
...
\hspace{1cm} Loop for each \(s \in \mathcal{S}\):

\hspace{1.5cm} \(v \leftarrow V(s)\)

\hspace{1.5cm} \(V(s) \leftarrow \sum_{s', r} p(s', r \mid s, \pi(s)) [r + \gamma V(s')]\)

\hspace{1.5cm} \(\Delta \leftarrow \max(\Delta, \vert v -  V(s) \vert)\)

\hspace{.5cm} until \(\Delta < \theta\) (a small positive number determining the accuracy of estimation)

3. Policy Improvement

\hspace{.5cm} \(\textit{policy-stable} \leftarrow true\)

\hspace{.5cm} For each \(s \in \mathcal{S}\):

\hspace{1cm} \(\textit{old-action} \leftarrow \pi(s)\)

\hspace{1cm} \(\pi(s) \leftarrow \underset{a}{argmax} \sum_{s', r} p(s', r \mid s, a) [r + \gamma V(s')]\)

\hspace{1cm} if \(\textit{old-action} \neq \pi(s)\), then \(\textit{policy-stable} \leftarrow false\)

\hspace{.5cm} if \(\textit{policy-stable}\), then stop and return \(V \approx v_*\) and \(\pi \approx \pi_*\); else go to 2.
...
\textbf{Value Iteration, for estimating deterministic policy \(\pi \approx \pi_*\)}:
...
\hspace{1cm} \(V(s) \leftarrow \max_a \sum_{s', r} p(s', r \mid s, a) [r + \gamma V(s')]\)

\hspace{1cm} \(\Delta \leftarrow \max(\Delta, \vert v -  V(s) \vert)\)

until \(\Delta < \theta\)

Output a deterministic policy, \(\pi \approx \pi_*\), such that: \(\pi(s) = \underset{a}{argmax} \sum_{s', r} p(s', r \mid s, a) [r + \gamma V(s')]\)
...
Asynchronous DP algorithms are in-place iterative DP algorithms that are not organized in terms of systematic sweeps of the state set. These algorithms update the values of states in any order whatsoever, 
...
Asynchronous algorithms also make it easier to intermix computation with real-time interaction. To solve a given MDP, we can run an iterative DP algorithm at the same time that an agent is actually experiencing the MDP. 
...
All of DP methods update estimates of the values of states based on estimates of the values of successor states. That is, they update estimates on the basis of other estimates. 
...
DP may not be practical for very large problems, but compared with other methods for solving MDPs, DP methods are actually quite efficient. If we ignore a few technical details, 
...
In practice, DP methods can be used with today's computers to solve MDPs with millions of states. Both policy iteration and value iteration are widely used, and it is not clear which, if either, is better in general. 
...
On problems with large state spaces, asynchronous DP methods are often preferred. To complete even one sweep of a synchronous method requires computation and memory for every state. 
...
Monte Carlo Methods estimates value functions and discovers optimal policies, without assuming complete knowledge of the environment. 
...
\hspace{.5cm} \(V(s) \in \mathbb{R}\) arbitrarily for all \(s \in \mathcal{S}\)

\hspace{.5cm} \(Returns(s)\), an empty list, for all \(s \in \mathcal{S}\)

Loop forever (for each episode):

\hspace{.5cm} Generate an episode following \(\pi\): \(S_0, A_0, R_1, S_1, A_1, R_2, \dots, S_{T - 1}, A_{T - 1}, R_{T}\)

\hspace{.5cm} \(G \leftarrow 0\)

\hspace{.5cm} Loop for each step of episode, \(t = T - 1, T - 2, \dots, 0\):

\hspace{1cm} \(G \leftarrow \gamma G + R_{t + 1}\)

\hspace{1cm} Unless \(S_t\) appears in \(S_0, S_1, \dots, S_{t - 1}\):

\hspace{1.5cm} Append \(G\) to \(Returns(S_t)\):

\hspace{1.5cm} \(V(S_t) \leftarrow average(Returns(S_t))\)
...
The policy evaluation problem for action values is to estimate \(q_{\pi}(s, a)\), the expected return when starting in state \(s\), taking action \(a\), 
...
For policy evaluation to work for action values, we must assure continual exploration. One way to do this is by specifying that the episodes start in a state-action pair, 
...
\hspace{.5cm} \(\pi(s) \in \mathcal{A}(s)\) (arbitrarily), for all \(s \in \mathcal{S}\)

\hspace{.5cm} \(Q(s, a) \in \mathbb{R}\) (arbitrarily), for all \(s \in \mathcal{S}\), \(a \in \mathcal{A}(s)\)

\hspace{.5cm} \(Returns(s, a)\), an empty list, for all \(s \in \mathcal{S}\), \(a \in \mathcal{A}(s)\)

Loop forever (for each episode):

\hspace{.5cm} Choose  \(S_0 \in \mathcal{S}\), \(A_0 \in \mathcal{A}(S_0)\) randomly such that all pairs have \(probability > 0\)

\hspace{.5cm} Generate an episode from \(S_0, A_0\), following \(\pi\): \(S_0, A_0, R_1, S_1, A_1, R_2, \dots, S_{T - 1}, A_{T - 1}, R_{T}\)
...
\hspace{1cm} Unless the pair \(S_t, A_t\) appears in \(S_0, A_0, S_1, A_1, \dots, S_{t - 1}, A_{t - 1}\):

\hspace{1.5cm} Append \(G\) to \(Returns(S_t, A_t)\):

\hspace{1.5cm} \(Q(S_t, A_t) \leftarrow average(Returns(S_t, A_t))\)

\hspace{1.5cm} \(\pi(S_t) \leftarrow \underset{a}{argmax} Q(S_t, a)\)
...
In on-policy control methods the policy is generally soft, meaning that \(\pi(a \mid s) > 0\) for all \(s \in \mathcal{S}\) and all \(a \in \mathcal{A}(s)\), 
...
\textbf{On-policy first-visit MC control (for \(\epsilon\)-soft policies), estimates \(\pi \approx \pi_*\)}:

Algorithm parameter: small \(\epsilon > 0\)

Initialize:

\hspace{.5cm} \(\pi \leftarrow\) an arbitrary \(\epsilon\)-soft policy

\hspace{.5cm} \(Q(s, a) \in \mathbb{R}\) (arbitrarily), for all \(s \in \mathcal{S}\), \(a \in \mathcal{A}(s)\)

\hspace{.5cm} \(Returns(s, a)\), empty list, for all \(s \in \mathcal{S}\), \(a \in \mathcal{A}(s)\)
...
\hspace{1.5cm} \(A^* \leftarrow argmax_{a} Q(S_t, a)\), with ties broken arbitrarily

\hspace{1.5cm} For all \(a \in \mathcal{A}(S_t)\):
...
In essence, we have shown that policy iteration works for \(\epsilon\)-soft policies. Using the natural notion of greedy policy for \(\epsilon\)-soft policies, 
...
On-policy methods are generally simpler and are considered first. Off-policy methods
...
Almost all off-policy methods utilize importance sampling, a general technique for estimating expected values 
...
Recall that we wish to estimate the expected returns (values) under the target policy, but all we have are returns $G _ { t }$ 
...
Now we are ready to give a Monte Carlo algorithm that averages returns from a batch of observed episodes following policy $ b $ 
...
or zero if the denominator is zero. To understand these two varieties of importance sampling, 
...
The every-visit methods for ordinary and weighed importance sampling are both biased, though, again, the bias falls asymptotically to zero 
...
\(p_{data}(o_t)\): Distribution of training data. The probability of taking action \(a_t\) at observation \(o_t\).

\(p_{\pi_{\theta}}(o_t)\): Distribution of validation data. The probability of taking correct action \(a_t\) at observation \(o_t\).

A standard training objective. Choose the optimal \(\theta\) that maxmizes the expectation of log probability of taking correct action \(a_t\) at observation \(o_t\), where \(o_t\) is sample from \(p_{data}(o_t)\):

\[\underset{\theta}{max} E_{o_t \sim p_{data}(o_t)}[log \pi_{\theta}(a_t \mid o_t)]\]
\[\underset{\theta}{max} \sum_{t = 1}^{m} log \pi_{\theta}(a_t \mid o_t)\]

An alternative training objective. We can either maxmize the reward:

\[r(o, a) = log p(a = \pi^*(o) \mid o)\]
\[\underset{\theta}{max} E_{o_t \sim p_{\pi_{\theta}}(o_t)}[r(o_t, a_t)]\]
\[\underset{\theta}{max} \sum_{t = 1}^{m} r(o_t, a_t)\]

Or, minimize the expect cost (number of mistakes) that the policy gonna make:
...
Imitation learning only learn from exsisting human behaviour, doesn't search for new approach. 
...
\textbf{Training data}: get \((s_{i}, a_{i}, R(s_{i}), s_{i+1})\) experience tuple from environment.
...
\textbf{Random (stochastic) environment?}
...
\hspace{.5cm} Receive initial state \(s_1\).

\hspace{.5cm} for \(t \in \{1, \dots, T\}\):

\hspace{1cm} Choose \(a_{t}\) for \(s_{t}\) with greedy policy \(\epsilon\).

\hspace{1cm} Get \((s_{t}, a_{t}, R(s_{t}), s_{t+1})\) experience tuple from environment.

\hspace{1cm} Store experience tuple in memory buffer, first in first out.

\hspace{1cm} Every \(C\) time steps perform a learning update:

\hspace{1cm} Sample \(B\) random experiences from memory buffer.

\hspace{1cm} For \((s_{j}, a_{j}, R(s_{j}), s_{j+1})\), \(j \in \{1, \dots, B\}\), construct training sample:
...
\hspace{1cm} Train \(Q_{new}\) such that \(Q_{new}(s, a) \approx y\):

\hspace{1cm} Perform a gradient descent with mini batch and implement soft update:

\[w = (1 - \eta)w + \eta w_{new}\]
\[b = (1 - \eta)b + \eta b_{new}\]

\hspace{1cm} Start a new episode if \(s_{t+1}\) terminates, else continue.

\section{Policy Gradient}

\textbf{On-policy vs Off-policy learning}:
...
\textbf{Policy Gradient Algorithms}:
...
\textbf{UCT (Upper Confidence Bound 1 applied to trees):}
